import { useEffect } from "react";
import { useForm, Controller } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { InputField } from "../../../components/InputField";
import { MultiSelectForm } from "../../../components/MultiSelectForm";
import { type UpdateAreaInput, updateAreaSchema } from "../schemas/updateAreaSchema";
import { useGetGrades } from "../../administrar-niveles/hooks";
import { useUpdateArea } from "../hooks";
import type { AreaWithGrades } from "../types/area";
import type { Grade } from "../../AdministratorUsers";

interface UpdateAreaModalProps {
  isOpen: boolean;
  onClose: () => void;
  area: AreaWithGrades;
}

export const UpdateAreaModal = ({ isOpen, onClose, area }: UpdateAreaModalProps) => {
  const {
    register,
    handleSubmit,
    control,
    reset,
    formState: { errors, isSubmitting },
  } = useForm<UpdateAreaInput>({
    resolver: zodResolver(updateAreaSchema),
    defaultValues: {
      name: area.name,
      description: area.description ?? "",
      is_group: area.is_group ?? false,
      gold: area.medal_parameter?.gold ?? 0,
      silver: area.medal_parameter?.silver ?? 0,
      bronze: area.medal_parameter?.bronze ?? 0,
      honor_mentions: area.medal_parameter?.honor_mentions ?? 0,
      grades: area.grades ?? [],
    },
  });

  const { mutateAsync: updateArea, error: errorUpdate } = useUpdateArea();
  const { data: gradesData } = useGetGrades();

  const gradeOptions = gradesData
    ? gradesData.map((grade) => ({
      label: grade.name,
      value: grade.id,
    }))
    : [];

  useEffect(() => {
    if (area) {
      reset({
        name: area.name,
        description: area.description ?? "",
        is_group: area.is_group ?? false,
        gold: area.medal_parameter?.gold ?? 0,
        silver: area.medal_parameter?.silver ?? 0,
        bronze: area.medal_parameter?.bronze ?? 0,
        honor_mentions: area.medal_parameter?.honor_mentions ?? 0,
        grades: (area.grades as Grade[])?.map((g) => g.id) ?? [],
      });
    }
  }, [area, reset]);

  const onSubmit = async (data: UpdateAreaInput) => {
    await updateArea({
      id: area.id,
      data: {
        name: data.name,
        description: data.description,
        is_group: data.is_group,
        gold: data.gold ?? undefined,
        silver: data.silver ?? undefined,
        bronze: data.bronze ?? undefined,
        honor_mentions: data.honor_mentions ?? undefined,
        grades: data.grades,
      },
    });
    reset();
    onClose();
  };

  const handleClose = () => {
    reset();
    onClose();
  };

  if (!isOpen) return null;

  const medalLabels: Record<string, string> = {
    gold: "Oro",
    silver: "Plata",
    bronze: "Bronce",
    honor_mentions: "Menciones de Honor",
  };

  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
        <h2 className="text-lg font-semibold mb-4">Editar Área</h2>

        <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-3">
          <InputField
            label="Nombre"
            type="text"
            {...register("name")}
            error={errors.name?.message}
            required
            disabled={isSubmitting}
            placeholder="Nombre del área"
          />

          <InputField
            label="Descripción"
            type="text"
            {...register("description")}
            error={errors.description?.message}
            disabled={isSubmitting}
            placeholder="Descripción del área"
          />

          <MultiSelectForm
            name="grades"
            label="Grados"
            control={control}
            options={gradeOptions}
            placeholder="Selecciona los grados"
            error={errors.grades}
            className="w-full"
          />

          <div className="border-t pt-4 mt-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">
              Parámetros de Medallas
            </h3>

            <div className="grid grid-cols-2 gap-3">
              {["gold", "silver", "bronze", "honor_mentions"].map((field) => (
                <Controller
                  key={field}
                  name={field as keyof UpdateAreaInput}
                  control={control}
                  render={({ field: { value, onChange } }) => (
                    <InputField
                      label={medalLabels[field]}
                      type="text"
                      onlyNumbers
                      value={value?.toString() ?? "0"}
                      onChange={(e) => {
                        const val = e.target.value;
                        if (/^\d*$/.test(val)) onChange(val === "" ? 0 : Number(val));
                      }}
                      disabled={isSubmitting}
                      error={errors[field as keyof UpdateAreaInput]?.message}
                    />
                  )}
                />
              ))}
            </div>
          </div>

          <div className="border-t pt-4 mt-4">
            <div className="flex items-center mb-3">
              <input
                type="checkbox"
                id="is_group"
                {...register("is_group")}
                className="w-4 h-4 text-primary-dark border-gray-300 rounded focus:ring-blue-500"
                disabled={isSubmitting}
              />
              <label
                htmlFor="is_group"
                className="ml-2 block text-sm font-medium text-gray-700"
              >
                Área de grupo
              </label>
            </div>
          </div>

          {errorUpdate?.response?.data?.message && (
            <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-sm font-semibold text-red-700 mb-1">
                Error al actualizar el área
              </p>
              <p className="text-sm text-red-600">{errorUpdate.response.data.message}</p>
            </div>
          )}

          <div className="flex justify-end gap-3 mt-5">
            <button
              type="button"
              onClick={handleClose}
              className="px-4 py-2 rounded-lg border border-gray-300 hover:bg-gray-100"
              disabled={isSubmitting}
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="px-4 py-2 rounded-lg bg-primary-dark text-white hover:bg-primary disabled:opacity-50"
              disabled={isSubmitting}
            >
              {isSubmitting ? "Actualizando..." : "Actualizar"}
            </button>
          </div>
        </form>
      </div>
    </div>
  );
};